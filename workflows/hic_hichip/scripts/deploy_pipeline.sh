...
Create a Hi-C / HiChIP Snakemake project directory.
...
The script links reusable pipeline files:
    Snakefile, rules/, envs/, scripts/, external/, run_snakemake.sh
...
mkdir -p "$PROJECT_DIR/config" "$PROJECT_DIR/raw_data" "$PROJECT_DIR/tools"
...
Next steps:
  ...
  2. Edit $PROJECT_DIR/config/config.yml and $PROJECT_DIR/config/samples.tsv
  3. Optional: edit $PROJECT_DIR/config/run_snakemake.env for local runtime settings
  4. Optional for FitHiChIP: cd "$PROJECT_DIR" && ./scripts/install_fithichip.sh --prefix tools/FitHiChIP
  5. Dry-run: cd "$PROJECT_DIR" && ./run_snakemake.sh -n --cores 1
EOF
